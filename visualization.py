...
def gen_visualization(image, keep_indices):
    ...
    # image_tokens = image.reshape(28, 28, 8, 8, 3).reshape(784, 8, 8, 3)
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./visualization/', help='path where to save')
...
@torch.no_grad()
def visualize_single_img(attentions, img_input, model, device, transform, post_process, save_name):
    important_tokens = 700
    alpha = 0.1
    # model.eval()
    ...
    #     vis_dict = model.information
    # changed
    attn_dict = trans(alpha, attentions)
    # vis_dict = attentions
    # vis_dict_0 = vis_dict[0].reshape(16, 4, -1).mean(dim=1).reshape(-1)
    # vis_dict = vis_dict_0 * alpha + (1 - alpha) * vis_dict[1].reshape(-1)

    image_raw = image_raw * 255
    image_raw = image_raw.squeeze(0).permute(1, 2, 0).cpu().numpy()

    # sorted_index = torch.argsort(vis_dict, descending=True)[:important_tokens]
    ...
    save_image_tensor(viz,
                      Path(args.output_dir, '{}_{}.jpg'.format(save_name, important_tokens)))
